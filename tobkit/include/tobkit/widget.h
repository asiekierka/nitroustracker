...
public:
	...
	Widget(u16 _x, u16 _y, u16 _width, u16 _height, Screen *screen,
	       bool _visible = true, bool _occluded = true);
	virtual ~Widget(void) {}
	...
	virtual void setTheme(Theme *theme_, u16 bgcolor_)
	{
		theme = theme_;
		bgcolor = bgcolor_;
	}
...
protected:
	...
	void drawString(const char *str, u16 tx, u16 ty, u16 color,
	                u16 maxwidth = 255, u16 maxheight = 255);
	...
	void drawHorizontalGradient(u16 col1, u16 col2, u16 tx, u16 ty, u16 tw,
	                            u16 th);

	inline u16 interpolateColor(u16 col1, u16 col2,
	                            int alpha /* 0..4095 */) const
	{
		return RGB5A1((((RGB5A1_R(col1) - RGB5A1_R(col2)) * alpha) +
		               (RGB5A1_R(col2) << 12)) >>
		                  12,
		              (((RGB5A1_G(col1) - RGB5A1_G(col2)) * alpha) +
		               (RGB5A1_G(col2) << 12)) >>
		                  12,
		              (((RGB5A1_B(col1) - RGB5A1_B(col2)) * alpha) +
		               (RGB5A1_B(col2) << 12)) >>
		                  12,
		              1);
	}

	void drawMonochromeIcon(u16 tx, u16 ty, u16 tw, u16 th, const u8 *icon,
	                        u16 color);
	void drawMonochromeIconOffset(u16 tx, u16 ty, u16 tw, u16 th, u16 ix,
