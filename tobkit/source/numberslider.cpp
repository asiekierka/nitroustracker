...
NumberSlider::NumberSlider(u16 _x, u16 _y, u16 _width, u16 _height,
                           Screen *_screen, s32 _value, s32 _min, s32 _max,
                           bool _hex, bool _is_8bit)
    : Widget(_x, _y, _width, _height, _screen), value(_value), btnstate(0),
      min(_min), max(_max), hex(_hex), is_8bit(_is_8bit)
{
	onChange = 0;
	onPostChange = 0;
}
...
void NumberSlider::draw(void)
{
	...
	char numberstr[5];
	if (is_8bit) {
		snprintf(numberstr, sizeof(numberstr), hex ? "%2hhx" : "%3hhd",
		         (int)value);
	} else {
