...
.macro BEGIN_ASM_FUNC name
    section = text arch = unset.section.\section\().\name\(),
    "ax", % progbits.global \name.type \name,
    % function

                .if \arch ==
        thumb.balign 2 .thumb.elseif \arch == arm.balign 4 .arm.elseif \arch ==
        unset
            // If the architecture it isn't specified, don't set it in this macro.
            // Set the aligment to the one that will work in all cases.
            .balign 4 .else.error "Unknown architecture".endif \name
    :.endm
            ...
            // anywhere else.
            .macro BEGIN_ASM_FUNC_NO_SECTION name arch =
        unset.global \name.type \name,
    % function
