...
SingleCellSetAction::SingleCellSetAction(State *state, u8 channel, u16 row,
                                         Cell cell_)
...
MultipleCellSetAction::MultipleCellSetAction(State *state, u8 channel, u16 row,
                                             CellArray *array_, bool clone)
...
{
	if ((array == NULL) ||
	    !validateCoords(song, array->width(), array->height()))
		return false;

	for (int x = 0; x < array->width(); x++)
		for (int y = 0; y < array->height(); y++) {
			Cell *ptr = &song->getPattern(
			    song->getPotEntry(potpos))[channel + x][row + y];
...
MultipleCellSetAction *newCellClearAction(State *state, Song *song, u16 x1,
...
CellInsertAction::CellInsertAction(State *state, u8 channel, u16 row,
                                   u16 width_, u16 height_)
    : PatternLocatedAction(state, channel, row)
{
...
bool CellInsertAction::revert(Song *song)
{
	...
	// restore
	array->paste(ptn, song->getChannels(), ptn_len, channel,
	             ptn_len - 1 - array->height());

	return true;
}

CellDeleteAction::CellDeleteAction(State *state, u8 channel, u16 row,
                                   u16 width_, u16 height_)
    : PatternLocatedAction(state, channel, row)
{
	array = new CellArray(width_, height_);
...
ActionBuffer::ActionBuffer(int size) : on_change([] {}), action_size(size)
{
...
bool ActionBuffer::undo(Song *song)
{
	...
	sassert(actions[undo_pos] != NULL, "action is null! (%d %d)", a_head,
	        a_tail);
...
bool ActionBuffer::redo(Song *song)
{
	...
	int redo_pos = a_pos;
	sassert(actions[redo_pos] != NULL, "action is null! (%d %d)", a_head,
	        a_tail);
